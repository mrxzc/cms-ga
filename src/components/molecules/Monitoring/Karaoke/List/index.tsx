...
import { IMonitoringKaraokeList } from '@interfaces/monitoringKaraoke'
import { useGetLocation } from '@services/gcm/location/query'
import { useGetRoomFloor } from '@services/gcm/roomFloor/query'
import { useGetListKaraoke, useGetListKaraokeMonitoringStatus } from '@services/monitoring/karaoke/query'
import { KaraokeMonitoringStatusClassEnum } from '@services/monitoring/karaoke/enums'
...
export function List() {
  ...
  const statusEnums = new KaraokeMonitoringStatusClassEnum()
  ...
  const { data, isFetching } = useGetListKaraoke(handleMappingParams())
  // Fetch List Karaoke
  ...
  const { data: status, isFetching: isStatusFetching } = useGetListKaraokeMonitoringStatus(statusParams)
  // Fetch List Status
    ...
    <div className="mb-[600px]">
      <div className="px-4 py-8">
        <div className="bg-white px-6 py-3 rounded mb-4 flex justify-between">
          <div className="text-extra-small regular-12">Monitoring Pesanan - Karaoke</div>
          <div className="flex">
        ...
        <div className="bg-white rounded-lg mb-4 p-6 relative">
          <div className="text-heading s semibold-18 mb-6">Monitoring Pesanan - Karaoke</div>
              ...
              <table className="table-fixed custom-table">
                ...
                <tbody className="table-body text-paragraph regular-14">
                  ...
                  {!isFetching && data?.data?.length ? (
                    data?.data?.map((val: IMonitoringKaraokeList) => (
                        ...
                        <td className="text-center min-w-[100px] w-full">
                          <div className="flex justify-end mr-3">
                            <button
                              ...
                              className="mr-3"
                              onClick={() => {
                                router.push(`/monitoring/karaoke/${val?.idBooking}`)
                              }}
                            >
                    ...
                    <tr>
                      <td colSpan={10} className="relative w-full h-48 min-w-[1830px] justify-center"></td>
                    </tr>
                  )}
                </tbody>
              </table>
