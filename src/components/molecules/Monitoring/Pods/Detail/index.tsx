...
import { useGetDetailPods } from '@services/monitoring/pods/query'
import { useParams, useRouter } from 'next/navigation'

export function Detail() {
  const router = useRouter()

  const paramsPage = useParams<{ pods: string }>()
  ...
  const {
    ...
    isRefetchError,
    refetch,
  } = useGetDetailPods({ noIdBooking: paramsPage?.pods })
      ...
      <div className="px-4 py-8 ">
        <div className="bg-white px-6 py-3 rounded mb-4 flex items-center space-x-2">
          <button
            type="button"
            onClick={() => {
              router.push('/monitoring/pods')
            }}
          >
            <div className="text-extra-small regular-12 text-[#235696]">Monitoring Pesanan - Pods</div>
          </button>
          <IconChevronRight color={'#909090'} width={24} height={24} className="-mt-0.5" />
          <div className="text-extra-small regular-12 text-[#252525]">Pods Detail</div>
        </div>

        <div className="bg-white rounded-lg mb-4 p-6 relative">
          <p className="text-heading s semibold-18 mb-10">Request Meeting Pods</p>
        ...
        <div className="bg-white rounded-lg mb-4 p-6 relative">
          <p className="text-heading s semibold-18 mb-10">Detail Pods yang dipesan</p>
