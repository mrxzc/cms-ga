...
import { mutateDeleteCarType, mutateToggleStatusCarType } from '@services/gcm/carType/mutation'
...
export function List() {
  ...
  const [selectedCarType, setSelectedCarType] = useState<IGcmCarType>()
  ...
  const { mutate: mutateToggle, isSuccess: isToggleSuccess, reset: toggleReset } = mutateToggleStatusCarType()
  const {
    mutate: mutateDelete,
    ...
    <div className="mb-[600px]">
      <div className="px-4 py-8">
        <div className="bg-white px-6 py-3 rounded mb-4 flex justify-between">
          <div className="text-extra-small regular-12">Master Data - Manage Tipe Mobil</div>
          <div className="flex">
            ...
            <button
              ...
              onClick={() => router.push('/master/car-type/add')}
            >
        ...
        <div className="bg-white rounded-lg mb-4 p-6 relative">
          <div className="text-heading s semibold-18 mb-6">Master Data Tipe Mobil</div>
            ...
            <div className="relative mb-6">
              <div className="rounded-lg border border-[#E6E5E6] overflow-auto">
                <table className="table-fixed custom-table">
                  ...
                  <tbody className="table-body text-paragraph regular-14">
                    {data?.data?.map((carType: IGcmCarType, index, arr) => (
                      <tr
                        key={`carType-${carType?.noSr}`}
                        className={`${index != arr.length - 1 ? 'border-b border-[#E6E5E6]' : ''}`}
                      >
                        <td className="min-w-[80px]">{carType?.noSr}</td>
                        <td className="min-w-[300px]">{carType?.descGcm}</td>
                        <td className="min-w-[300px]">{carType?.brand}</td>
                        <td className="text-center min-w-[200px] ">{carType?.dtUpload}</td>
                        <td className="text-center min-w-[100px]">
                          <label className="switch">
                            <input
                              name={`checkbox-${carType?.noSr}`}
                              defaultChecked={carType?.flagActive}
                              type="checkbox"
                              onChange={val => {
                                const payload: IGcmCarTypeToggleStatusPayload = {
                                  noSr: carType?.noSr,
                        ...
                        <td className="text-center min-w-[100px] w-full">
                          <div className="float-right">
                            <button
                              ...
                              className="mr-3"
                              onClick={() => {
                                router.push(`/master/car-type/${carType?.noSr}/edit`)
                              }}
                            >
            ...
            <div className="bg-white relative p-6 text-center rounded-xl justify-center">
              ...
              <div className="flex justify-center items-center space-x-3">
                ...
                <button
                  ...
                  onClick={() => {
                    if (selectedCarType) {
                      const payload: IGcmCarTypeDeletePayload = {
