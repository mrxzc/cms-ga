...
import { IGcmCarBrandCreateForm, IGcmCarBrandCreatePayload } from '@interfaces/gcmCarBrand'
import { useMutateCreateCarBrand } from '@services/gcm/carBrand/mutation'
...
export function Add() {
  ...
  const {
    ...
    reset: createReset,
  } = useMutateCreateCarBrand()

  const { handleSubmit, control, formState } = useForm<IGcmCarBrandCreateForm>({
  ...
  const onSubmit = (form: IGcmCarBrandCreateForm) => {
    const payload: IGcmCarBrandCreatePayload = { ...form }
    ...
    <div className="mb-[600px]">
      <div className="px-4 py-8 ">
        <div className="bg-white px-6 py-3 rounded mb-4 flex items-center space-x-2">
          ...
          <div className="text-extra-small regular-12 text-[#252525]">Add Brand Mobil</div>
        </div>

        <div className="bg-white rounded-lg mb-4 p-6 relative">
          <p className="text-heading s semibold-18 mb-10">Add Brand Mobil</p>
