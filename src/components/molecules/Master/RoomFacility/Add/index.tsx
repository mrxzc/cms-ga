...
import { IGcmRoomFacilityCreateForm, IGcmRoomFacilityCreatePayload } from '@interfaces/gcmRoomFacility'
import { useMutateCreateRoomFacility } from '@services/gcm/roomFacility/mutation'
...
export function Add() {
  ...
  const {
    ...
    reset: createReset,
  } = useMutateCreateRoomFacility()

  const { handleSubmit, control, formState } = useForm<IGcmRoomFacilityCreateForm>({
  ...
  const onSubmit = (form: IGcmRoomFacilityCreateForm) => {
    const payload: IGcmRoomFacilityCreatePayload = { ...form }
  ...
  useEffect(() => {
    if (isCreateSuccess) {
      setTimeout(() => {
        createReset()
        router.push('/master/room-facility')
      }, 3000)
    }
  }, [isCreateSuccess])
    ...
    <div className="mb-[600px]">
      <div className="px-4 py-8 ">
        <div className="bg-white px-6 py-3 rounded mb-4 flex items-center space-x-2">
          ...
          >
            <div className="text-extra-small regular-12 text-[#235696]">Master Data - Manage Fasilitas Ruangan</div>
          </button>
          <IconChevronRight color={'#909090'} width={24} height={24} className="-mt-0.5" />
          <div className="text-extra-small regular-12 text-[#252525]">Add Fasilitas Ruangan</div>
        </div>

        <div className="bg-white rounded-lg mb-4 p-6 relative">
          <p className="text-heading s semibold-18 mb-10">Add Fasilitas Ruangan</p>
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
            ...
            <div className="flex justify-end gap-2 items-end text-heading xs regular-16">
              <button
                ...
                type="button"
                onClick={() => router.push('/master/room-facility')}
              >
                Cancel
              </button>
              <button
