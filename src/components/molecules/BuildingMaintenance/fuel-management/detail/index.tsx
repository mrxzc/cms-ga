...
import { useGetBuildingManagementFuelDetail } from '@services/building-management/fuel/query'
...
export function Detail() {
  ...
  const {
    ...
    isRefetchError,
    refetch,
  } = useGetBuildingManagementFuelDetail({ noId: paramsPage?.wasteManagement })
      ...
      <div className="px-4 py-8 ">
        <div className="bg-white px-6 py-3 rounded mb-4 flex items-center space-x-2">
          <button
            type="button"
            onClick={() => {
              router.push('/building-management/ehs/fuel-management')
            }}
          >
            <div className="text-extra-small regular-12 text-[#235696]">Building Maintenance - Pengunaan BBM</div>
          </button>
          <IconChevronRight color={'#909090'} width={24} height={24} className="-mt-0.5" />
          <div className="text-extra-small regular-12 text-[#252525]">Pengunaan BBM Detail</div>
        </div>

        <div className="bg-white rounded-lg mb-4 p-6 relative">
          <p className="text-heading s semibold-18 mb-10">Request Pengunaan BBM</p>
          ...
          {isFetchSuccess && meta?.data && (
            <div>
              <div>
                ...
                <div className="grid grid-cols-4 gap-4 mb-6 items-center">
                  <p className="text-heading xs regular-16">BBM Existing</p>
                  <div className="col-span-3">
                    <div className="text-paragraph regular-14 text-[#717171] border border-[#E6E5E6] bg-[#EFF2F5] min-h-[44px] p-3 rounded-md ">
                      {meta?.data?.bbmExisting} m3
                    </div>
                  </div>
                </div>

                <div className="grid grid-cols-4 gap-4 mb-6 items-center">
                  <p className="text-heading xs regular-16">BBM Request</p>
                  <div className="col-span-3">
                    <div className="text-paragraph regular-14 text-[#717171] border border-[#E6E5E6] bg-[#EFF2F5] min-h-[44px] p-3 rounded-md ">
                      {meta?.data?.bbmRequest} m3
                    </div>
                  </div>
                </div>
