...
export function Management() {
  ...
  const columns = [
    columnHelper.accessor('originalIndex', {
      cell: info => {
        const offset = ((assets?.pagination?.currentPage ?? 1) - 1) * 10
    ...
    columnHelper.accessor('ACTION', {
      cell: info => {
        ...
        const handleEditClick = () => {
          router.push(`/management/asset/edit-asset/${rowData.noIdAsset}`)
        }

        const handleDeleteClick = () => {
          setAssetIdToDelete(rowData.noIdAsset)
          setIsModalOpen(true)
        }
  ...
  const handleDeleteData = async (id: string | null) => {
    if (id) {
      try {
        const response = await apiDeleteAsset({ assetId: id })
      ...
      <div className="px-4 py-8 bg-[#f6f6f6] h-full w-full overflow-auto">
        ...
        <div className="bg-white px-4 py-4 rounded-xl">
          ...
          <Table
            ...
            pagination={{
              TOTAL_DATA: assets?.pagination?.totalRecords ?? 0,
              PAGE: assets?.pagination?.currentPage ?? 1,
              LAST_PAGE: assets?.pagination?.totalPage ?? 1,
            }}
        ...
        <div className="p-4 bg-white rounded relative flex flex-col items-center">
          ...
          <div className="flex justify-center gap-4 items-end">
            ...
            <button
              ...
              type="button"
              onClick={() => handleDeleteData(assetIdToDelete)}
            >
              Ya, Hapus
            </button>
          </div>
